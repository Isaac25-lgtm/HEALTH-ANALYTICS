...
Fresh databases upgrade empty → `0012_population_staging_identity` (head), followed by `python scripts/bootstrap_reference_data.py` for the reference configuration (or `scripts/bootstrap_db.py` with `SEED_DEV_DATA=true` for synthetic development data). A database at any earlier revision upgrades forward in order; revisions 0001–0008 are frozen. Do not assume a shared database can be deleted. Revision 0004 adds programme scope to raw aggregate uniqueness and enforces one open-ended current geometry per organisation unit; reconcile pre-existing duplicate current rows before upgrading. Revision 0005 adds export file metadata and AI request columns.

...
| `HPIP_PYTHON` / `HPIP_BROWSER_EXECUTABLE` | Playwright acceptance gate: Python for the disposable API and optional local browser executable |
| `E2E_EXIT_BOUND_SECONDS` / `E2E_INACTIVITY_TIMEOUT_SECONDS` / `E2E_OVERALL_TIMEOUT_SECONDS` / `E2E_SERVICE_STOP_TIMEOUT_MS` / `E2E_BUILD_TIMEOUT_SECONDS` / `E2E_PROCESS_QUERY_TIMEOUT_MS` / `E2E_PROCESS_SETTLE_TIMEOUT_MS` | Playwright gate lifecycle bounds; defaults 60 s after summary, 90 s without pre-summary output, 600 s overall Playwright runtime, 10 s per owned service stop, 900 s for the production build, 20 s per process-table query and 5 s of bounded survivor quiescence. Failure to establish the initial process inventory prevents the build and all service/browser launches. |
| `HPIP_E2E_EXTERNAL_SERVERS` | Internal gate-to-Playwright switch; disables Playwright's shell-owned web servers because the gate owns direct child processes. Do not set for `e2e:raw`. |
| `HPIP_REUSE_E2E_SERVER` | Diagnostic `e2e:raw` only: opt in to Playwright's built-in server reuse |

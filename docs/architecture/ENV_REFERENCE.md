...
| `E2E_EXIT_BOUND_SECONDS` / `E2E_INACTIVITY_TIMEOUT_SECONDS` / `E2E_OVERALL_TIMEOUT_SECONDS` / `E2E_SERVICE_STOP_TIMEOUT_MS` / `E2E_BUILD_TIMEOUT_SECONDS` / `E2E_PROCESS_QUERY_TIMEOUT_MS` | Playwright gate lifecycle bounds; defaults 60 s after summary, 90 s without pre-summary output, 600 s overall Playwright runtime, 10 s per owned service stop, 900 s for the production build and 20 s per process-table query (a timed-out query fails closed) |

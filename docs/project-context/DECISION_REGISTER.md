...
- The canonical national root is the active, parentless organisation unit with code `UG`. Another
  active country root is a reference conflict, and administrator landing never selects an
  unordered country row.
- The Playwright acceptance command owns its API, Next and test-runner processes directly. Process
...
| D-057 | Aggregate DHIS2 history is kept from July 2020 onward (`RAW_AGGREGATE_RETENTION_DAYS=3650`, `CALCULATION_SNAPSHOT_RETENTION_MONTHS=132` in the local operator environment). MPDSR case-level retention stays short and unchanged. Months are the single stored source; financial years, quarters, calendar years and custom ranges are materialised from them (`sum_of_months`, with component row IDs) and only when every month of the period was extracted. | Owner decision 2026-09-24 |
| D-058 | Custom month ranges (`YYYYMM..YYYYMM`, at most 72 months) are supported. Their population denominator is blended month by month from the approved population rows; if any month has no approved population the denominator is unavailable, never zero. Before FY2024/25 no approved population exists, so coverage percentages are unavailable while counts remain. | Owner decision 2026-09-24 |
| D-059 | EPI bands, applied as governed revision `v2-epi-bands` of the 24 vaccine-coverage and the dropout indicators: coverage green >= 90%, amber 80-89.9%, red < 80%; dropout green < 10%, red >= 10%. Ranking of immunisation indicators follows these bands. | Owner decision 2026-09-24 |
| D-060 | District/city boundaries (`UGANDA_DISTRICT.json`, SHA-256 `5a6024f5...27b8`) attach to the live 146-unit DHIS2 district/city cohort. Matching is exact normalised name, with the DHIS2 " District" suffix removed for district-type units only, plus two owner-approved spelling aliases: LUWEERO -> Luwero District and SSEMBABULE -> Sembabule District. 146/146 matched, 0 ambiguous. Reference `OWNER-DECISION-2026-09-24-D060`. | Owner decision 2026-09-24; applied |
| D-061 | Boundary effective date is 1 July 2020 (reference `OWNER-DECISION-2026-09-24-D061`), covering the whole stored history. | Owner decision 2026-09-24; applied |
| D-062 | Sub-counties attach directly to their district; the DHIS2 level-4 DLG/Municipality layer is not modelled. Not yet imported. | Owner decision 2026-09-24 |
| D-063 | MPDSR programme and stage fields are discovered read-only from DHIS2 and presented as a short candidate list for owner confirmation; no MPDSR field is guessed. | Owner decision 2026-09-24; discovery done 2026-09-25, candidates in `MPDSR_FIELD_CANDIDATES.md` await confirmation |
| D-064 | Data-quality spike/drop checks compare a period only with the immediately preceding period of the same kind (previous month, previous financial year; a custom range with the same window a year earlier), using the approved ratio (3.0). Comparing a financial-year total with single months produced thousands of false flags. | Correction 2026-09-24 (defect fix, rule threshold unchanged) |
